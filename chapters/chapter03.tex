...
\end{align}
    where each \(\sigma^{(\natural)}_a \cong \mathbb{B}^k\) and \(\sigma^{(.)}_a \cap X^{k-1} = \varphi_a(S^{(\natural)-1})\).
...
\end{equation}
where $H(\natural)$ generally represents either the $k$-th dimensional
