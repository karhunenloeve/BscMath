...
\]
for a finite filtration\index{filtration}, where each $V_{i} := \langle \{\sigma^{(-)}_a\}_{a \in I_0}, \{\sigma^{(-)}_a\}_{a \in I_1}, \ldots, \{\sigma^{(-)}_a\}_{a \in I_i}
