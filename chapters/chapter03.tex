...
\begin{proof}
Each submodule \( C^{\perp i} \) is defined as the dual of the quotient \( C^n / C^{n-i} \), i.e., $C^{\perp i} = (C^n / C^{n-i})^\sharp$. Thus, the persistence module \( (C^{\perp n} / \mathcal{C}^\perp(X)) \) is constructed by taking the quotient of \( C^{\perp d} \) by \( \mathcal{C}^\perp(X) \), which gives the reversed filtration\index{filtration}:
\begin{align}
...
\begin{proof}
We use Theorem \ref{persistencepartition} and obtain a partition of indices \( \{0, 1, 2, \ldots, n\} = B \sqcup B^{\Join} \sqcup D \) for \( \mathcal{C}^\perp(X) \) and new generators \( \hat{\tau}_i \), where:
\begin{align}
