...
\cup \bigcup_{a \in I_d} \sigma_a^{(d)}$, adding a single cell at a time for each filtration step, so that the persistence module can be expressed as the
sequence
\[
	\mathcal{C}: \quad C_{1} \to C_{2} \to \cdots \to C_{d},
\]
for a finite filtration\index{filtration}, where each $C_{i} := \langle \{\sigma^{(1)}_a\}_{a \in I_1}, \ldots, \{\sigma^{(i)}_a\}_{a \in I_i}
