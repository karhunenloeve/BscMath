...
\[
	H_{\bullet}(\mathcal{X}) = H(\mathcal{C}(X), \partial): \quad \frac{\ker(\partial_{1})}{\text{im}(\partial_{1})}
...
= \partial^{\flat}$ is defined as the adjoint of $\partial$. Then $C^{\flat}(\mathcal{X}) = (\mathcal{C}^{\flat}(X), \delta)$ and
$H^{\bullet}(\mathcal{X}) = H(\mathcal{C}^{\flat}(X), \delta) = \ker(\delta) / \text{im}(\delta)$. This is a persistence module, with morphisms in the
reverse direction.
...
\begin{example}{\cite[\S 2.6, Example]{de2011dualities}}
	...
	\begin{align}
		(C_{d}/\mathcal{C}(X)):         & \quad C_{d} \rightarrow (C_{d}/C_{1}) \rightarrow (C_{d}/C_{2}) \rightarrow \cdots \rightarrow (C_{d}/C_{d-1}),                             \\
		(C_{d}/\mathcal{C}(X))^{\flat}: & \quad C_{d}^{\flat} \leftarrow (C_{d}/C_{1})^{\flat} \leftarrow (C_{d}/C_{2})^{\flat} \leftarrow \cdots \leftarrow (C_{d}/C_{d-1})^{\flat}.
	\end{align}
...
\end{tikzcd}
\caption{A persistent chain complex, where moving to the right increases the filtration\index{filtration} index, while moving downwards decreases the dimension. The maps $f_{i,k}^{i+1}$ correspond to the structure map from the $i$th to the $(i+1)$st chain group over $\mathbb{Z}$ -- or vector space if we work over $\F$ -- in filtration step $k$. The boundary operator is defined for the chain complex, restricting it for each element to the respective chain group.}
\end{figure}
