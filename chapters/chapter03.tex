...
\begin{itemize}
    ...
    \item The barcode\index{barcode} $\mathcal{B}(\mathcal{C})$ provides a visual and combinatorial representation of the intervals and their multiplicities. Each interval in the barcode\index{barcode} corresponds to a range of points $k$ on the real line where the persistence module maintains consistent behavior. A spectral point indicates a transition between these behaviors, which corresponds to the endpoints of intervals in the barcode\index{barcode}.
...
\begin{example}{\cite[\S 2.3, Example]{de2011dualities}}
    ...
    the persistence diagram of the absolute homology\index{homology} of a sphere, specifically $\mathcal{S}^{1}$:
    \begin{align}
...
\begin{example}{\cite[\S 2.4, Example]{de2011dualities}}
	...
	the homology\index{homology} class is $[\sigma_{3}] = [\sigma_{4}]$. This class vanishes in $H_{1}(S^{5},S^{2})$, thus we yield the interval $[1,2)$.
\end{example}
...
can be described via a filtered CW-complex $\mathcal{X}:= \bigcup_{k=0}^{d} \sigma_k^{(k)}$, adding a single cell for each filtration step, so that the persistence module is the sequence
\[
	\mathcal{C}(X): \quad C_{0} \to C_{1} \to C_{2} \to \cdots \to C_{d},
\]
for a finite filtration\index{filtration}, where each $C_{k}^{i} := \langle \sigma_0^{(k)}, \sigma_1^{(k)}, \ldots, \sigma_i^{(k)} \rangle$ is a vector space (or an abelian group) over a field $\F$, generated by the $k$-simplizes $\sigma^{(k)}_i$. The boundary operator is defined by $\partial_{k} \sigma^{(k)}_{j} = \sum_{i<j}\lambda_{ij}\sigma^{(k-1)}_{i}$ for $\lambda_{ij}\in \F$ for all $i, j \in \{0, 1, \ldots, d\}$. Then $\mathcal{C}(X) = \{C_{k},\partial_{k}\}_{k=0}^{d}$ is the chain complex of $X$, and $\mathcal{C}(\mathcal{X})$ represents the persistent version for $\mathcal{X}$. This leads to the definition of the persistent absolute homology\index{homology} of $\mathcal{X}$ as
\[
...
\]
where $C^{i\flat}_k = \text{Hom}(C^{i}_k, \F) = \langle \sigma_0^{(k)\flat}, \sigma_1^{(k)\flat}, \ldots, \sigma_i^{(k)\flat} \rangle$, with the elements $\sigma^{(k)\flat}_i$ as the dual basis of $\sigma^{(k)}_i$. The coboundary $\delta= \partial^{\flat}$ is defined as the adjoint of $\partial$. Considering $(\mathcal{C}^{\flat}(X), \delta)$, its homology forms a module structure $H^{\bullet}(\mathcal{X}) = H(\mathcal{C}^{\flat}(\mathcal{X}), \delta) = \ker(\delta) / \text{im}(\delta)$. This is a persistence module, with morphisms in the reverse direction.
...
\begin{theorem}[Partition of persistence modules]
	{\cite[Theorem 2.6]{de2011dualities}} \label{persistencepartition} Let $\mathcal{C}(\mathcal{X})$ be given and $\partial$ as above, then there
	exists a partition $\{0,1,2, \ldots, d\} = B^{\Join} \sqcup B \sqcup D$ with a bijective pairing $B \leftrightarrow D$, such that $b$ is paired with $d$ if and only if $[b,d) \in \operatorname{Pairs}(\mathcal{C}(\mathcal{X}), \partial)$. Furthermore, there is a basis
	$\hat{\sigma}^{(k)}_{0}, \hat{\sigma}^{(k)}_{1}, \hat{\sigma}^{(k)}_{2}, \ldots, \hat{\sigma}^{(k)}_{d}$ of $C^{d}_k$ such
	that
	\begin{itemize}
		\item $C^{i}_k = \langle \hat{\sigma}^{(k)}_{0}, \hat{\sigma}^{(k)}_{1}, \ldots, \hat{\sigma}^{(k)}_{i} \rangle$ for
			each $i$.
		\item $\partial_{k}\hat{\sigma}^{(k)}_{b} = 0$ for all $b \in B^{\Join}$.
		\item $\partial_{k}\hat{\sigma}^{(k)}_{d} = \hat{\sigma}^{(k)}_{b}$, and thus
			$\partial_{k}\hat{\sigma}^{(k)}_{b} = 0$, for all $[b,d) \in \operatorname{Pairs}$.
	\end{itemize}
	It follows that the persistence diagram
	$\operatorname{Pers}(H(\mathcal{C}(\mathcal{X}),\partial))$ consists of $[a_{b}, \infty)$ for
	$b \in B^{\Join}$ together with intervals $[a_{b},a_{d})$ for
	$[b,d) \in \operatorname{Pairs}$.
\end{theorem}

\begin{proof}
	Let $\mathcal{C}(\mathcal{X})$ be a finite filtered chain complex associated with
	a topological space, structured over a field $\F$. The filtration\index{filtration} indices ${0, 1, 2, \ldots, d}$
	classify the stages at which elements are born or die in homology\index{homology}. The
	partition of $\{0, 1, 2, \ldots, d\}$ into $B^{\Join} \sqcup B \sqcup D$ is then constructed such that $B^{\Join}$ contains indices corresponding to elements that persist indefinitely, i.e., those for which $\partial_{k}\sigma^{(k)}_{i} = 0$ and $\sigma^{(k)}_{i}$ does not become a boundary at any higher index. $B$ and $D$ are paired bijectively, where each $b \in B$ (births) corresponds uniquely to an $d \in D$ (deaths), signifying the termination of the homological feature introduced at $d$. This pairing is determined through the matrix reduction of $\partial$, ensuring that every cycle born at stage $b$ becomes a boundary at stage $d$. We then construct a basis ${\hat{\sigma}^{(k)}_i}$ such that for each $i$, $\hat{\sigma}^{(k)}
	_{i}$ is a generator of $C^{i}_k$. Specifically, if $i \in B^{\Join}$, then
	$\partial_{k}\hat{\sigma}^{(k)}_{i} = 0$, indicating these elements are cycles that persist
	indefinitely. For pairs $[b, d) \in \operatorname{Pairs}(\mathcal{C}(\mathcal{X}), \partial)$
	with $b \in B$ and $d \in D$, set $\partial_{k}\hat{\sigma}^{(k)}_{d} = \hat{\sigma}^{(k)}_{b}$
	and $\partial_{k}\hat{\sigma}_{b} = 0$, reflecting the fact that the cycle
	$\hat{\sigma}^{(k)}_{b}$ born at $b$ becomes a boundary at $d$ and thus ceases to
	contribute to homology\index{homology} past $d$. Hence, the persistence diagram $\operatorname{Pers}(H(\mathcal{C}(\mathcal{X}), \partial))$ contains
...
In Theorem \ref{persistencepartition} the index sets can be reexpressed in the original language of positive and negative simplices proposed by Edelsbrunner et al. \cite[p.8]{de2011dualities}, where a $k$-simplex is called positive if it belongs to a $k$-cycle and negative if it doesn't, respectively:
\begin{itemize}
...
\end{itemize}
The simplices $\hat{\sigma}^{(k)}_{b}$ and $\hat{\sigma}^{(k)}_{d}$ are cycles characterized by
their leading terms $\sigma^{(k)}_{b}$ and $\sigma^{(k)}_{d}$ respectively, while the simplex
$\hat{\sigma}^{(k)}_{d}$ is a chain with leading term $\sigma^{(k)}_{d}$. This chain 'kills'
the homology\index{homology} class of its paired cycle $\hat{\sigma}^{(k)}_{b}$ through the boundary relation
$\partial_{k} \hat{\sigma}^{(k)}_{d} = \hat{\sigma}^{(k)}_{b}$.
\end{remark}
