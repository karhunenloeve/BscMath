...
\]
for a finite filtration\index{filtration}, where each $C_{i} := \langle \{\sigma^{(\natural)}_a\}_{a \in I_0}, \{\sigma^{(\natural)}_a\}_{a \in I_1}, \ldots, \{\sigma^{(\natural)}_a\}_{a \in I_i}
\rangle$ is a vector space (or an abelian group) over a field $\F$, generated by the elements $\sigma^{(\natural)}_a$ for $a \in I_0, I_1, \ldots, I_i$. The boundary operator
...
\begin{example}{\cite[\S 2.6, Example]{de2011dualities}}
	...
	$(C_{d}/\mathcal{C}(X))$ are surjective. Thus, absolute and relative cohomology\index{cohomology}
...
To capture the algebraic structure of this filtration\index{filtration}, we consider the free graded module \( \mathcal{C}(X) \) over the polynomial ring \( \mathbb{F}[t] \) with generators \( \sigma^{(\natural)}_0, \sigma^{(\natural)}_1, \ldots, \sigma^{(\natural)}_i \) corresponding to the basis elements of each chain complex \( C_i(X_j) \). The action of \( t \) on \( \mathcal{C}(X) \) encodes the structure maps (or inclusions) \( f_{i,j}^{i+1} \) by shifting indices: multiplying by \( t \) represents moving from \( C_i(X_j) \) to \( C_{i+1}(X_j) \).

The boundary operator \( \partial: \mathcal{C}(X) \to \mathcal{C}(X) \) is compatible with the module: $\partial_\natural(\sigma^{(\natural)}_i) \in C_{i-1} \subseteq \mathcal{C}(X)$, ensuring \( \partial^2_{\natural+1} = 0 \) and making \( \partial_\natural \) a graded map that commutes with multiplication by \( t \).

The homology\index{homology} \( H(\mathcal{C}(X), \partial) \) is computed as $H_k(\mathcal{C}(X), \partial) = \ker(\partial_k) / \ima(\partial_{k+1})$, where \( \partial_k: C_k(X_i) \to C_{k-1}(X_{i+1}) \). Thus, \( H_k(\mathcal{C}, \partial) \) captures precisely the data of the persistent homology\index{homology}\index{persistent homology\index{homology}} groups \( H_k(X_i \subseteq X_j) \) for all \( i \leq j \). Thus, $H(\mathcal{C}, \partial) \cong \bigoplus_{0 \leq i \leq j \leq n} H_k(X_i \subseteq X_j)$.
