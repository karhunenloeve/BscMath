...
\begin{align}
	((C^{b^{\Join}})^n/\mathcal{C}(X)^{b^\Join})&: \quad \langle \hat{\sigma}_{b^\Join}\rangle \rightarrow 0,\nonumber\\
...
\end{align}
It follows that $H((C^{bd})^n/\mathcal{C}(X)^{bd})$ contributes a single interval of the form $[a_b,a_d)$, generated by the equivalence class $[\hat{\sigma}_b]$, and thus has one dimension more than $[a_b,a_d) \in \operatorname{Pers}(H(\mathcal{C}(X)))$, which is generated by $[\hat{\sigma}_d]$.
\end{proof}
...
\end{align}
where we set \( C^{\perp i} := (C^n / C^{n-i})^\sharp \). It is already known that \( \sigma_0^\sharp, \sigma_1^\sharp \ldots, \sigma_n^\sharp \) form a basis of \( C^{\sharp n} \), dual to the generators \( \sigma_0, \sigma_1, \ldots, \sigma_n \) of \( C^n \). We define \( \tau_i = \sigma^\sharp_{n+1-i} \), leading to
\begin{align}
