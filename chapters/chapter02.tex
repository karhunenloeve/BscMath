...
We have a correspondence of $A \subset \tilde{K}$ with $\sigma^{(d)} \in K$ and $B \subset \tilde{K}$ with $\tau^{(k)} \in K$. The abstract simplicial complex $\tilde{K}$ associated with a simplicial complex
...
\begin{proof}
	...
	\end{align}	
	Since each \( K_i \) is a connected component, by Proposition \ref{decomp}, each \( H^{\triangle}_{0}(K_i; \mathbb{Z}) \) is isomorphic to \( \mathbb{Z} \). Therefore, we have $H^{\triangle}_{0}(K; \mathbb{Z}) \cong \bigoplus_{i=1}^p \mathbb{Z} = \mathbb{Z}^{p}$.
\end{proof}

\newpage
\begin{example}\noindent
	\begin{enumerate}
		\item The zeroth homology group of the circle is isomorphic to \(\mathbb{Z}\). Consider a simplicial representation of the circle using four \(1\)-simplices: \(\sigma_{0}^{(1)} = (v_{0}, v_{1})\), \(\sigma_{1}^{(1)} = (v_{1}, v_{2})\), \(\sigma_{2}^{(1)} = (v_{2}, v_{3})\), and \(\sigma_{3}^{(1)} = (v_{3}, v_{0})\). The group \(C^{\triangle}_{0}\) consists of sums over the four zero-simplices \(v_{0}, v_{1}, v_{2}, v_{3}\) with coefficients in \(\mathbb{Z}\), called zero-chains. Let \(c\) be a general zero-chain with non-zero coefficients:
		\begin{align}
		    c = g_{0} v_{0} + g_{1} v_{1} + g_{2} v_{2} + g_{3} v_{3}.
		\end{align}
		The zeroth homology group \(H^{\triangle}_{0}\) is computed by looking at cycles, i.e., zero-chains that are not boundaries. Since there are no lower-dimensional simplices, all zero-chains are cycles. However, we identify zero-chains under equivalence by adding boundaries of $1$-chains. We can express a general boundary of a $1$-chain as:
		\begin{align}
			c' &= a_{0} \sigma_{0}^{(1)} + a_{1} \sigma_{1}^{(1)} + a_{2} \sigma_{2}^{(1)} + a_{3} \sigma_{3}^{(1)}, \nonumber\\
		    \partial_{1}(c') &= a_{0}(v_{1} - v_{0}) + a_{1}(v_{2} - v_{1}) + a_{2}(v_{3} - v_{2}) + a_{3}(v_{0} - v_{3}),
		\end{align}
		where \( a_0, a_1, a_2, a_3 \in \mathbb{Z} \). The boundary operator \(\partial_1\) takes any 1-chain to a combination of differences of vertices. Thus, for the zero-chain \(c\), we can reduce it by adding a boundary \( \partial_{1}(a_{0} \sigma_{0} + a_{1} \sigma_{1} + a_{2} \sigma_{2} + a_{3} \sigma_{3}) \), leading to the equivalence: $c \sim (g_{0} + g_{1} + g_{2} + g_{3}) v_{0}$. Since the space is connected, all vertices \(v_0, v_1, v_2, v_3\) are homologous. This implies that the entire chain can be reduced to a single vertex, say \(v_0\), with a total coefficient \(g = g_{0} + g_{1} + g_{2} + g_{3}\). Therefore, we can write the equivalence class of \(c\) as $c \sim g v_{0}$, where \(g \in \mathbb{Z}\) is the sum of the original coefficients. Thus, \(H^{\triangle}_{0} \cong \mathbb{Z}\), generated by the equivalence class of any single vertex, indicating that the zeroth homology group corresponds to the number of connected components, which in this case is one.      		      		      
		\item We will demonstrate that \(H^{\triangle}_{d}(S^{d}) \cong \mathbb{Z}\). The \(d\)-simplex \(\sigma^{(d)}\) and the \(d\)-ball are homeomorphic, and their boundaries, which consist of \((d-1)\)-simplices, are homeomorphic to the \(d\)-sphere \(S^d\). Therefore, an appropriate simplicial structure for \(S^d\) is the boundary of the \((d+1)\)-simplex \(\sigma^{(d+1)}\).  Let \(\{v_0, v_1, \dots, v_{d+1}\}\) denote the set of vertices of \(\sigma^{(d+1)}\). These vertices are unordered, but we can impose orientations on the \((d-1)\)-simplices using their numbering to establish an orientation convention.  Thus, a general \(d\)-chain on this structure is of the form:
		\begin{align}
		    \label{chain}
		    c = \sum_{i=0}^{d+1} g_i (v_0, \ldots, v_{i-1}, v_{i+1}, \ldots, v_d),
		\end{align}
		where \(g_i \in \mathbb{Z}\). The \(d\)-simplices here are oriented by omitting one vertex at a time from the complete set \(\{v_0, v_1, \dots, v_{d+1}\}\). The boundary operator \(\partial_{d+1}\) applied to a \(d\)-chain sums the boundaries of each \(d\)-simplex by removing a vertex from each simplex. Specifically:
		\begin{align*}
		    \partial_{d+1}(c) &= \partial_{d+1} \left( \sum_{i=0}^{d+1} g_i (v_0, \ldots, v_{i-1}, v_{i+1}, \ldots, v_d) \right) \nonumber\\
		    &= \sum_{i=0}^{d+1} g_i \left( \sum_{j=0}^{d} (-1)^j (v_0, \ldots, v_{i-1}, v_{i+1}, \ldots, v_{j-1}, v_{j+1}, \ldots, v_d) \right).
		\end{align*}
		The result is a sum over \((d-1)\)-simplices formed by removing one more vertex from each \(d\)-simplex, with alternating signs depending on the orientation. To simplify, observe that each \((d-1)\)-simplex appears twice, with opposite orientations. For example, consider the boundary of two adjacent simplices sharing a face, such as \((v_0, v_1, \ldots, v_d)\) and \((v_1, v_2, \ldots, v_{d+1})\). The face shared by these two simplices appears with opposite orientations in the boundary of each. Thus, when summing over the boundary of the entire \((d+1)\)-simplex, all interior faces cancel out, leaving only the boundary of the boundary - i.e., nothing $\partial_{d+1}(c) = 0$. Since \(\partial_{d+1}(c) = 0\), every \(d\)-chain \(c\) is a cycle, meaning \(c \in Z^{\triangle}_{d}\). However, we now need to determine the group of boundaries \(B^{\triangle}_{d}\) to compute the homology group \(H^{\triangle}_d = Z^{\triangle}_d / B^{\triangle}_d\). There are no \((d+1)\)-chains in this structure, as \(S^d\) is the boundary of a \((d+1)\)-simplex, so there are no higher-dimensional simplices that could give rise to nontrivial boundaries. Therefore, \(B^{\triangle}_d = 0\), implying that every cycle is a homology class. Thus, we have $H^{\triangle}_{d}(S^d) = Z^{\triangle}_{d}$. Now, let’s simplify the \(d\)-chain \(c\) itself. The boundary condition implies that the coefficients of the simplices must all be equal, i.e., \(g_0 = g_1 = \dots = g_{d+1}\). This is because any difference in coefficients would result in nonzero contributions to the boundary. Thus, the chain reduces to:
		\begin{align}
		    c = g_0 \sum_{i=0}^{d+1} (v_0, \ldots, v_{i-1}, v_{i+1}, \ldots, v_d).
		\end{align}
		Since \(g_0\) is an arbitrary integer, the group of \(d\)-chains is isomorphic to \(\mathbb{Z}\), and we conclude $H^{\triangle}_d(S^d) \cong \mathbb{Z}$.     		      
