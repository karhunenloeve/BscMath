...
\begin{theorem}[Excision]
	...
	\begin{align}
		H_{d}(X\setminus Z, A\setminus Z) \to H_{d}(X, A) \quad \text{for all }n. 
	\end{align}
...
\begin{proof}
	...
	\begin{align}
		...
		\end{cases}
	\end{align}
	The \(n\)-simplices thus form the structure of a simplicial complex. The diameter of a simplex is defined as the maximum distance between its vertices. Consider a point \(v\) and a point of the form \(\sum_i t_i v_i\) within the simplex \([v_0, \ldots, v_n]\). The distance between these two points satisfies the inequality
	\begin{align}
	...
	\item Barycentric subdivision of linear chains \cite[Proposition 2.21 (2)]{hatcher2005algebraic}: We construct now the subdivision operator $S: C_n(X) \rightarrow C_n(X)$ and show, that it is chain homotopic to the identity map. For a convex set \(Y\) in Euclidean space, the linear maps \(\lambda: \sigma^{(n)} \rightarrow Y\) generate a subgroup of \(C_n(Y)\), which we denote by \(LC_n(Y)\), the so-called linear chains\index{chains}. The boundary\index{boundary} operator \(\partial: C_n(Y) \rightarrow C_{n-1}(Y)\) maps \(LC_n(Y)\) to \(LC_{n-1}(Y)\), hence the linear chains\index{chains} form a subcomplex of the singular chain complex of \(Y\). We define the unique map \(\lambda: \tilde{\sigma}^{(n)} \rightarrow Y\) by \([w_0, \ldots, w_n]\), where \(w_i\) is the image of the \(i\)-th vertex of \(\tilde{\sigma}^{(n)}\) under \(\lambda\). To avoid the constant case distinction regarding the \(0\)-simplices, we complete the complex \(LC(Y)\) by defining \(LC_{-1}(Y) = \mathbb{Z}\), generated by the equivalence class of the empty simplex \([\emptyset]\), with \(\partial[w_0] = [\emptyset]\) for all \(0\)-simplices \([w_0]\).

	Let us now go into more detail. Consider the \(n\)-simplex \(\tilde{\sigma}^{(n)}\), defined as convex hull of its vertices \(\{v_0, \ldots, v_n\}\) in a topological space. A linear map \(\lambda: \tilde{\sigma}^{(n)} \rightarrow Y\) is determined by the images of the vertices \(v_i\), and thus \(\lambda\) is entirely defined by the points \(w_i = \lambda(v_i)\) in \(Y\). The image \(\lambda(\tilde{\sigma}^{(n)})\) is a linear simplex in \(Y\), and the subgroup \(LC_n(Y)\) consists of formal sums of such linear simplices. The boundary\index{boundary} of a linear simplex \([w_0, \ldots, w_n]\) is given by the alternating sum
	\begin{align}
