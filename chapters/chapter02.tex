...
\begin{proof}
	...
	We define $T$ on $LC_n(Y)$ inductively by setting $T=0$ for $n=-1$ and $T\lambda = b_\lambda(\lambda-T\partial\lambda)$ for $n \geq 0$. This formula is an inductive definition of the barycentric subdivision of $\Delta^n \times \mathbb{I}$ obtained by the union of all simplices in $\Delta^n \times \{0\} \cup \partial \Delta^n \times \mathbb{I}$ to the barycentre of $\Delta^n \times \{1\}$. The chain homotopy formula is $\partial T + T\partial = \mathbb{I} - S$. This is trivial for $LC_{-1}(Y)$, where $T=0$ and $S=\mathbb{I}$. The formula for $LC_n(Y)$ for all $n \geq 0$ is given by:
	\begin{align}
	...
	\begin{align}
		\partial S\sigma & = \partial (\sigma_\sharp S\Delta^n) = \sigma_\sharp \partial (S\Delta^n) = \sigma_\sharp S(\partial \Delta^n) \\
						 & = \sigma_\sharp S\left(\sum_i (-1)^i \Delta^n_i\right)  = \sum_i (-1)^i \sigma_\sharp S(\Delta^n_i) \\ 
						 &= \sum_i (-1)^i S(\sigma \vert \Delta^n_i) = S\left(\sum_i (-1)^i \sigma \vert \Delta^n_i\right) = S(\partial \sigma).                 
	\end{align}
