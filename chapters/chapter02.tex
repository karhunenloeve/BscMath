...
\begin{proof}
	...
	\end{equation}
	The Five Lemma \ref{fivelemma} supports the isomorphism between $H_{k}^{\triangle}(X^{k}, X^{k-1})$ and $H_{k}(X^{k}, X^{k-1})$. Thus, we conclude that $H_{d}^{\triangle}(X^{k}, X^{k-1}) \rightarrow H_{d}(X^{k}, X^{k-1})$ is an isomorphism for all $d$, verifying the equivalence of simplicial and singular homology for all $k$ and for all $d$.
\end{proof}