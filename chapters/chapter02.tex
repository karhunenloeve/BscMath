...
\begin{definition}[Index]
	The index is defined as a map \( I: C_{0}(X) \to \mathbb{Z} \) through
	\begin{align}
...
\begin{proof}
	...
	\end{align}
	Our goal is to demonstrate that \( \ker(I) = \operatorname{im}(\partial_{1}) \). That is, for any \( 0 \)-chain \( c \), \( I(c) = 0 \) if and only if \( c \sim 0 \). Suppose \( I(c) = 0 \) for \( c = \sum_{i} g_{i} \tilde{\sigma}^{(0)} \). Since \( X \) is path-connected, we can express \( c \) as the boundary of a \( 1 \)-chain:
	\begin{align}
