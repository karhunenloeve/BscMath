...
\begin{proof}
	...
	\begin{align}
		\partial_{d} S_d\lambda & = \partial_d b_\lambda(S_{d-1}\partial_{d}\lambda),                                                                                                                    \\
		                  ...
		                  & = S_{d-1}\partial_d\lambda - b_\lambda S_{d-1}(\partial^2_d\lambda) \quad \text{since } \partial_{d-1} S_{d-1}(\partial_d\lambda) = S(\partial^2_d\lambda) \text{ for all } d, \\
	...
	\begin{equation}
			...
			\end{tikzcd}
	\end{equation}
	We define $T$ on $LC_d(Y)$ inductively by setting $T=0$ for $d=-1$ and $T\lambda = b_\lambda(\lambda-T\partial_d\lambda)$ for $d \geq 0$. This formula is an inductive definition of the barycentric subdivision of $\tilde{\sigma}^{(d)} \times \mathbb{I}$ obtained by the union of all simplices in $\tilde{\sigma}^{(d)} \times \{0\} \cup \partial \tilde{\sigma}^{(d)} \times \mathbb{I}$ to the barycentre of $\tilde{\sigma}^{(d)} \times \{1\}$. The chain homotopy formula is $\partial_{d+1} T + T\partial_d = \mathbb{I} - S$. This is trivial for $LC_{-1}(Y)$, where $T=0$ and $S=\mathbb{I}$. The formula for $LC_n(Y)$ for all $n \geq 0$ is given by:
	\begin{align}
		\partial_{d+1} T\lambda & = \partial_{d+1} b_\lambda(\lambda - T\partial_{d
		}\lambda),                                         \\
		                  & = \lambda - T\partial_d\lambda - b_\lambda\partial_d(\lambda - T\partial_{d-1}\lambda)              \\
