...
\begin{proof}
	...
	We define a characteristic map $\varsigma: \coprod_{i}(\sigma^{(k)}_{i}, \sigma^{(k-1)}_{i}) \rightarrow (X^{k}, X^{k-1})$, inducing a homeomorphism
	\[
		\varsigma_\ast: \coprod_{i} \sigma^{(k)}_{i}/\coprod_{i} \sigma^{(k-1)}_{i} \rightarrow X^{k}/X^{k-1}.
	\]
