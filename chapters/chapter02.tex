...
\begin{proof}
	...
	As a consequence, the subdivision homomorphism $S: LC_d(Y) \rightarrow LC_d(Y)$ can be defined inductively over $d$. Now let $\tilde{\sigma}^{(d)}$ be a generator of $LC_d(Y)$, and let $b_\lambda$ be the image of the barycentre of $\tilde{\sigma}^{(n)}$ under $\lambda$. We define the value of $S(\lambda)$ as $b_\lambda(S\partial_d\lambda)$, such that $b_\lambda: LC_{d-1}(Y) \rightarrow LC_d(Y)$. The beginning of the induction shows that $S([\emptyset]) = [\emptyset]$ applies, so that $S$ is the identity on $LC_{-1}(Y)$. Consequently, this mapping is also the identity on $LC_0(Y)$, as the following applies for $n=0$: 
	\begin{align}
