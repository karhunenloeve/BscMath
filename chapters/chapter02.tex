...
\begin{proof}
	...
	\item Barycentric subdivision of linear chains \cite[Proposition 2.21 (2)]{hatcher2005algebraic}: We construct now the subdivision operator $S: C_d(X) \rightarrow C_d(X)$ and show, that it is chain homotopic to the identity map. For a convex set \(Y\) in Euclidean space, the linear maps \(\lambda: \tilde{\sigma}^{(d)} \rightarrow Y\) generate a subgroup of \(C_d(Y)\), which we denote by \(LC_d(Y)\), the so-called linear chains\index{linear chains}. The boundary\index{boundary operator} operator \(\partial_d: C_d(Y) \rightarrow C_{d-1}(Y)\) maps \(LC_d(Y)\) to \(LC_{d-1}(Y)\), hence the linear chains\index{linear chains} form a subcomplex of the singular chain complex of \(Y\). We define the unique map \(\lambda: \tilde{\sigma}^{(n)} \rightarrow Y\) by \([w_0, \ldots, w_n]\), where \(w_i\) is the image of the \(i\)-th vertex of \(\tilde{\sigma}^{(n)}\) under \(\lambda\). To avoid the constant case distinction regarding the \(0\)-simplices, we complete the complex \(LC(Y)\) by defining \(LC_{-1}(Y) = \mathbb{Z}\), generated by the equivalence class of the empty simplex \([\emptyset]\), with \(\partial[w_0] = [\emptyset]\) for all \(0\)-simplices \([w_0]\). Let's go into more detail. Consider the \(d\)-simplex \(\tilde{\sigma}^{(d)}\), defined as convex hull of its vertices \(\{v_0, \ldots, v_d\}\) in a topological space. A linear map \(\lambda: \tilde{\sigma}^{(d)} \rightarrow Y\) is determined by the images of the vertices \(v_i\), and thus \(\lambda\) is entirely defined by the points \(w_i = \lambda(v_i)\) in \(Y\). The image \(\lambda(\tilde{\sigma}^{(d)})\) is a linear simplex in \(Y\), and the subgroup \(LC_d(Y)\) consists of formal sums of such linear simplices. The boundary\index{boundary} of a linear simplex \([w_0, \ldots, w_n]\) is given by the alternating sum
	\begin{align}
	...
	As a consequence, the subdivision homomorphism $S: LC_d(Y) \rightarrow LC_d(Y)$ can be defined inductively over $d$. Now let $\tilde{\sigma}^{(d)}$ be a generator of $LC_d(Y)$, and let $b_\lambda$ be the image of the barycentre of $\tilde{\sigma}^{(d)}$ under $\lambda$. We define the value of $S(\lambda)$ as $b_\lambda(S\partial_{d}\lambda)$, such that $b_\lambda: LC_{d-1}(Y) \rightarrow LC_d(Y)$. The beginning of the induction shows that $S([\emptyset]) = [\emptyset]$ applies, so that $S$ is the identity on $LC_{-1}(Y)$. Consequently, this mapping is also the identity on $LC_0(Y)$, as the following applies for $d=0$: 
	\begin{align}
		S([w_0]) = w_0(S(\partial_0[w_0])) = w_0(S([\emptyset])) = w_0([\emptyset]) = w_0. 
	\end{align}
	Provided that $\lambda$ is an embedding with $\text{ran}(\lambda) = [w_0, \ldots, w_d]$, $S(\lambda)$ can be defined as the sum of the $d$-simplices in the barycentric subdivision of $[w_0, \ldots, w_d]$. Since $S = \mathbb{I}$ applies to $LC_0(Y)$ and $LC_{-1}(Y)$, it follows for all further $d \in \mathbb{N}$ that
	\begin{align}
		\partial_{d} S\lambda & = \partial_d b_\lambda(S\partial_d\lambda),                                                                                                                    \\
		                  & = S\partial_d\lambda - b_\lambda\partial_d(S\partial_d\lambda) \quad \text{since } \partial_d b_\lambda = \mathbb{I} - b_\lambda\partial_d,                          \\
		                  & = S\partial_d\lambda - b_\lambda S(\partial^2_d\lambda) \quad \text{since } \partial_d S(\partial_d\lambda) = S(\partial^2_d\lambda) \text{ for all } d, \\
		                  & = S\partial_d\lambda.                                                                                     
	\end{align}
	Thus, the chain map $S: LC(Y) \rightarrow LC(Y)$ satisfies $\partial_d S = S\partial_d$.
		
	Next, we define a chain homotopy $T: LC_d(Y) \rightarrow LC_{d+1}(Y)$ between $S$ and the identity, such that the following diagram commutes:
	\begin{equation}
	...
	\end{align}
	Now we can drop the group\index{group} $LC_{-1}(Y)$ again, and the relation $\partial_d T + T\partial_{d+1} = \mathbb{I} - S$ still holds since $T$ is zero for $LC_{-1}(Y)$.
