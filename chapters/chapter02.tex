...
\begin{proof}
	...
	\end{align}
	where \(\hat{w_i}\) denotes the omission of the vertex \(w_i\). Since each face of a linear simplex is itself a linear simplex, \(\partial_d\) maps \(LC_d(Y)\) into \(LC_{d-1}(Y)\), ensuring that \(\mathcal{LC}_\bullet(Y)\) is a subcomplex of the singular chain complex \(\mathcal{C}\), for the two chain complexes
	\begin{align}
