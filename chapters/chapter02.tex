...
A $d$-cycle of a $k$-complex $K$ is said to be homologous to zero if it can be expressed
...
\begin{definition}
	In the context of a topological space $X$, a singular $d$-simplex\index{singular
	$d$-simplex} is defined as a map $\tilde{\sigma}^{(d)}: \sigma^{(d)}\to X$,
...
\begin{definition}
	The singular homology group\index{singular homology group} $H_{d}(X)$ is
...
Since the maps $\tilde{\sigma}^{(d)}$ are continuous, a singular simplex always
...
\begin{definition}
	...
	boundary operators\index{boundary operators}), such that the composition of
...
concept of exact sequences\index{exact sequence}.
...
\begin{theorem}[Equivalence of $H_{d}^{\Delta}$(X) and $H_{d}$(X)]
